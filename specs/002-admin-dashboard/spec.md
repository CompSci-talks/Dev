...
**Independent Test**: Can be tested by creating "Spring 2026", setting it as active, and viewing the public schedule to verify only Spring 2026 seminars are shown, while others move to the archive.
...
1. **Given** an existing seminar record, **When** I upload a video or PPT file, **Then** the file is streamed securely to the external storage drive.
2. **Given** a successful upload, **When** the external drive generates a File ID, **Then** that ID is saved to the seminar record in the database.
3. **Given** a file is uploaded to the external drive, **When** it is saved, **Then** the storage service's native restrictions (e.g., "disable download/print/copy") are automatically applied to the file.
...
### User Story 5 — Q&A Moderation (Priority: P2)

As an administrator, I want to review, hide, or delete inappropriate questions submitted to a seminar's Q&A section so that the public portal remains professional and focused.

**Why this priority**: Required for community safety and engagement quality, ensuring speakers aren't subjected to spam or abusive questions.

**Independent Test**: Can be tested by submitting a question as a regular user, logging in as an admin to hide it, and verifying it disappears from the regular user's view.

**Acceptance Scenarios**:

1. **Given** users have submitted questions to a seminar, **When** I review the seminar's Q&A section in the admin dashboard, **Then** I see a list of all submitted questions.
2. **Given** an inappropriate question exists, **When** I choose to "Hide" it, **Then** the question immediately disappears from the attendee portal but remains visible in the admin dashboard.
3. **Given** a hidden or inappropriate question exists, **When** I choose to "Delete" it, **Then** it is permanently removed from both the attendee portal and the database.
...
- **FR-005**: The system MUST route seminars with a future date to the public "Upcoming Schedule" if they fall within the Active Semester.
- **FR-006**: The system MUST route seminars with a past date directly to the "Previous Talks" archive.
...
- **FR-009**: The system MUST provide an interface for uploading video and presentation files.
- **FR-010**: The system MUST stream uploaded files to an external storage provider and retrieve a unique File ID.
- **FR-011**: The system MUST automatically configure the external storage provider to disable download, print, and copy permissions for the uploaded file.
- **FR-012**: The system MUST save the external storage File ID to the corresponding seminar record in the database.
...
- **FR-015**: The system MUST allow administrators to view all Q&A submissions for any seminar.
- **FR-016**: The system MUST allow administrators to toggle the visibility (Hide/Show) of individual Q&A questions.
- **FR-017**: The system MUST allow administrators to permanently delete individual Q&A questions.
...
- **Seminar**: (Shared with Phase 1) Extended attributes needed for admin management: linked semester ID.
- **Material**: (Shared with Phase 1) The admin interface manages the creation of these records and the API interaction to generate the external storage link.
...
- **SC-005**: An administrator can view the RSVP list and hide an inappropriate Q&A question in 3 clicks or fewer from the main dashboard.
