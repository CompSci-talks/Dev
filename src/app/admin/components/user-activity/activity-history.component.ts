...
@Component({
    ...
    <div class="flow-root">
      <ul role="list" class="-mb-8">
        <li *ngFor="let activity of activities; let last = last">
          <div class="relative pb-8">
            <span *ngIf="!last" class="absolute left-4 top-4 -ml-px h-full w-0.5 bg-gray-200 dark:bg-gray-700" aria-hidden="true"></span>
            <div class="relative flex space-x-3">
              <div>
                <span [class]="getIconClass(activity.type)" class="h-8 w-8 rounded-full flex items-center justify-center ring-8 ring-white dark:ring-gray-900">
              ...
              <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                <div>
                  <p class="text-sm text-gray-500 dark:text-gray-400">
                    ...
                    <span class="font-medium text-gray-900 dark:text-gray-100">{{ activity.metadata['title'] || activity.metadata['seminarTitle'] }}</span>
                  </p>
                  <p *ngIf="activity.metadata['content']" class="mt-1 text-sm text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-gray-800 p-2 rounded border border-gray-100 dark:border-gray-700">
      ...
      <div *ngIf="activities.length === 0" class="text-center py-10">
        <p class="text-sm text-gray-500 dark:text-gray-400">No activity history found for this user.</p>
      </div>
    </div>
  `
})
export class ActivityHistoryComponent {
    ...
    getIconClass(type: string): string {
        switch (type) {
            case 'seminar_attendance': return 'bg-green-500';
            case 'comment_posted': return 'bg-blue-500';
            case 'comment_replied': return 'bg-indigo-500';
            case 'profile_updated': return 'bg-yellow-500';
