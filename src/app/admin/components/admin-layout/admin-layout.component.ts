    ...
    <div class="flex bg-slate-50">
      ...
      <aside class="w-64 bg-slate-900 text-white flex flex-col flex-shrink-0 sticky top-0 h-screen">
        ...
        <div class="px-6 py-5 border-b border-slate-800">
          <p class="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-1">Admin Portal</p>
          <h1 class="text-lg font-bold text-white tracking-tight">CompSci Talks</h1>
        </div>
        ...
        <div class="px-4 py-4 border-t border-slate-800">
          <a routerLink="/" class="flex items-center gap-2 text-xs text-slate-400 hover:text-white transition-colors">
            ...
            <div class="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center">
              @if (auth.currentUser$ | async; as user) {
                @if (user.photo_url) {
                  ...
                  <span class="text-white text-xs font-bold">{{ user.display_name.charAt(0).toUpperCase() }}</span>
                }
              }
            </div>
          </div>
