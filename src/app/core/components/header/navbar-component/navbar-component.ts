    ...
    <header class="bg-surface-card border-b border-border shadow-sm sticky top-0 z-50">
      ...
      @if (mobileOpen()) {
        <div class="md:hidden border-t border-border bg-surface-card px-4 py-4 space-y-1">
          <a routerLink="/schedule" routerLinkActive="text-primary font-semibold bg-slate-50"
             (click)="closeMobile()"
          ...
          <a routerLink="/archive" routerLinkActive="text-primary font-semibold bg-slate-50"
             (click)="closeMobile()"
             class="flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-medium text-text-muted hover:text-primary hover:bg-slate-50 transition-colors">
            Archive
          </a>
          ...
          @if (auth.currentUser$ | async; as user) {
            ...
            @if (user.role === 'admin') {
              <a routerLink="/admin" routerLinkActive="text-primary font-semibold bg-slate-50"
                 (click)="closeMobile()"
                 class="flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-medium text-text-muted hover:text-primary hover:bg-slate-50 transition-colors">
                Admin Panel
              </a>
            ...
            <div class="border-t border-border mt-2 pt-2">
              ...
              <button (click)="logout()"
                      class="flex items-center gap-2 w-full px-3 py-2.5 rounded-lg text-sm font-medium text-red-500 hover:bg-red-50 transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/>
                </svg>
                Logout
